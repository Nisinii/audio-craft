...
def fine_tune_bert(dialogues, sentiments, epochs=3, batch_size=8, learning_rate=2e-5):
...
def evaluate_model(true_labels, predicted_labels, predicted_probabilities):
    ...
    # Convert val_probabilities to a NumPy array
    val_probabilities = np.array(predicted_probabilities)

    # Calculate ROC curve and AUC
    fpr, tpr, _ = roc_curve(true_labels, val_probabilities[:, 1], pos_label=1)
    roc_auc = auc(fpr, tpr)
    plt.figure()
